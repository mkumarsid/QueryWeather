...
#                         tooltip = row["station_id"]
#                         popup = f"{city}<br>🌡️ {row.get('Temperature', '?')} °C<br>💧 {row.get('Humidity', '?')} %"
#                         folium.Marker(location=coords, tooltip=tooltip, popup=popup).add_to(m)
...
#         should_fetch = st.button("📊 Compute Stats") or self._cache_key not in st.session_state
...
#                 st.session_state[self._cache_key] = (data, headers)
